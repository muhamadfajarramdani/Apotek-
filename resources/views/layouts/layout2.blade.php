        ...
        <div class="container">
            ...
            @if (Auth::check())
                <div class="collapse navbar-collapse" id="navbarNav">
                    ...
                    <form class="d-flex ms-auto" role="search" action="{{ route('kelola.akun') }}" method="GET">
                        <input type="text" class="form-control me-2" placeholder="Search Data Akun"
                            aria-label="Search" name="search_akun"  >
                        <button class="btn btn-outline-success me-2" type="submit">Search</button>
                    </form>
                    <button class="btn btn-success text-white ms-2 ">
                        <a class="text-white link-underline-success"
                            href="{{ route('kelola.akun', ['sort' => 'name', 'order' => request('order') == 'asc' ? 'desc' : 'asc']) }}">
                            @if (request('order') == 'asc' && request('sort') == 'name')
                                Terbesar
                            @else
                                Terkecil
                            @endif
